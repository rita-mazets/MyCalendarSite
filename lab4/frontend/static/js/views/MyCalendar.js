...
export default class extends AbstractView {
    ...
    async addBirthDate(name, year, month, day){
        document.getElementById('birthdate').textContent = name + ", You were born " + day + "." + month + "." + year
    }

    async changeActiveButton(birthdate){
        var now = new Date()
        var diff = (now - birthdate)/1000/60/60/24/7;
        let i = 1
        while(i < diff)
        {
    ...
    async addHandlers(){
        ...
        week.addEventListener('click', async event => {
            ...
            await getAssesmentInfo(id)/*.then(() =>
            {
                this.addPictures(id).then(() => navigateTo('/myCalendar'))
            })*/
        })
    ...
    upload(selector){
        ...
        const open = this.element('button', ['custom-button','accesmant-button'], 'Add pictures')
        open.id = 'add_btn'
